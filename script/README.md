...
上传完成后通常会得到 **两个** 根地址（域名可能是 `gateway.irys.xyz`、`node1.irys.xyz` 等，以你终端 **`Uploaded to`** 打印为准；**同一域名体系内**与下面规则一致即可）。

| 根 URL 含义 | 典型来源 | 是否作为工厂 `deploy*` 的 `baseURI` |
|------------|----------|--------------------------------------|
| **图片包**根（下面再拼 `/1.png`、`/2.png`…） | 上传 **`images`** 目录成功后 | **否**。`tokenURI` 需要能解析成 **JSON 元数据**，整条链上不能只指向纯 PNG。 |
| **元数据（JSON）包**根（下面再拼 `/1`、`/2`…，与本地无后缀文件名一致） | 上传 **`metadata`** 目录成功后 | **是**。工厂 `deployProxy` / `deployProxyDeterministic` 的第三个参数 **`baseURI`** 应填 **这一条**（再按需加末尾斜杠，见下段）。 |

**图片包根**只应出现在每条 metadata 里的 **`"image"`** 字段（由 `generate-nft-metadata.js` 写入）；**不要**把它当作合约里的 `baseURI`。

**末尾斜杠**：`NFTTemplate` 使用 OpenZeppelin 时，`tokenURI` 一般为 **`baseURI` 与 `tokenId` 的字符串直接拼接**，中间**不会**自动插入 `/`。因此 `baseURI` 建议写成带 **末尾斜杠** 的元数据根，例如：

`https://node1.irys.xyz/<元数据根ID>/`

这样 `tokenURI(1)` 为 `https://node1.irys.xyz/<元数据根ID>/1`，与网关上的单份 JSON 路径一致。

**一句话**：合约 / 工厂初始化用 **JSON（metadata）那条根**；**图片那条根**只活在 JSON 的 **`image`** 里。
